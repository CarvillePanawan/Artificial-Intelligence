...
public class GreedyBestFirstSearch {
    public static void main(String[] args) {
        try {
            GreedyBestFirstSearch amazingProgram = new GreedyBestFirstSearch();
            amazingProgram.run();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void run() {
        ...
        while(frontier.size() > 0) {
            ...
            if(currentState.isGoal(goal)) {
                ...
                ArrayList<State> successorStates = currentState.expandInformedB(temp);
    ...
    public static void showSolution(State state, int totalStatesVisited, int maxFrontierSize) {
        ...
        for (State st : path) {
            System.out.println("Misplace Tiles Cost: "+st.getHeuristicCost());
            st.showPuzzle();
        }
    }//showSolutions
}//class
